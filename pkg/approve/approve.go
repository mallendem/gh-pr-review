...
const (
	...
	cRed    = "\033[31m"
	cGreen  = "\033[32m"
	// Orange via 256-color; falls back gracefully on many terminals
...
func ManualApproval(user string, propagate bool, dryRun bool) error {
	...
	if userMap, ok := userHashPrMap[user]; ok {
		for h := range userMap {
			hashesMap[h] = struct{}{}
		}
	} else {
		...
		for uname, userMap := range userHashPrMap {
			if strings.EqualFold(uname, user) {
				for h := range userMap {
					hashesMap[h] = struct{}{}
				}
				break
	...
	for idx, h := range hashes {
		...
		if prs, ok := hashPrMap[h]; ok {
			...
			if skip {
				fmt.Printf("%s\n", colorize(cYellow, fmt.Sprintf("Skipping hash %s because one of its PRs was previously skipped", h)))
				continue
			}
		}
		...
		if changes, ok := changeMap[h]; ok {
			...
			if allDup && len(originals) > 0 {
				...
				sort.Strings(origs)
				fmt.Printf("%s\n", colorize(cYellow, fmt.Sprintf("All changes for hash %s are duplicates of %v and already approved — auto-approving.", h, origs)))
				continue
			}

			// Print changes, marking duplicates and then record firstSeen for previously unseen lines
			fmt.Println(colorize(cYellow, "Changes:"))
			for _, line := range changes {
				if first, seen := firstSeen[line]; seen {
			...
			fmt.Println(colorize(cYellow, "No changes recorded for this hash."))
		}
		// Show associated PRs
		prCount := 0
		firstPrKey := ""
		if prs, ok := hashPrMap[h]; ok {
			prCount = len(prs)
			fmt.Println(colorize(cYellow, "Associated PRs:"))
			for i, pr := range prs {
			...
			fmt.Println(colorize(cYellow, "No PRs associated with this hash."))
		}
		...
		for {
			fmt.Print(colorize(cOrange, fmt.Sprintf("pr %d/%d hash: %d/%d approve this hash? (y/n/q) ", prProgressIndex, totalPRs, idx+1, total)))
			...
			if input == "y" || input == "a" {
				...
				fmt.Println(colorize(cYellow, "Quitting manual approval early."))
				goto decide
			} else {
				fmt.Println(colorize(cYellow, "Please enter y (approve), n (decline) or q (quit)"))
			}
		}
	}
...
decide:
	...
	for prKey, phashes := range prMap {
		...
		if allApproved {
					...
					if pr.GetHTMLURL() == prKey {
						...
						if dryRun {
							...
							if err := g.ApprovePr(pr); err != nil {
								fmt.Printf("%s\n", colorize(cYellow, fmt.Sprintf("Failed to approve PR %s: %v", prKey, err)))
							} else {
			...
			if !found {
				fmt.Printf("%s\n", colorize(cYellow, fmt.Sprintf("Could not find PR object for %s to approve", prKey)))
			}
		}
	}
